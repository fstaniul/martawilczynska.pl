...
function reducer(state, current) {
  return { previous: state.selected, selected: current };
}

export default function TestimonialsDisplay() {
  const [{ selected, previous }, setCurrent] = useReducer(reducer, { selected: 0, previous: 0 });
