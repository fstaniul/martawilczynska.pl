...
export const DEFAULT_LOCALE = window.LOCALE || (localStorage && localStorage.getItem('locale')) || LOCALES[0];
...
export const PATH_TO_LOCALE = Object.keys(LOCALE_ROUTES).reduce((acc, locale) => {
  Object.keys(LOCALE_ROUTES[locale]).forEach(
    routeKey => (acc[LOCALE_ROUTES[locale][routeKey]] = { path: routeKey, locale })
  );
  return acc;
}, {});

export const localizePath = (locale, name) => (name === '/' ? '/' : LOCALE_ROUTES[locale][name]);

export const switchLocalePath = (locale, pathname) => {
  if (pathname === '/') return '/';
  const path = '/' + pathname.split('/')[1];
  const currentLocaleInfo = PATH_TO_LOCALE[path];
  return currentLocaleInfo && LOCALE_ROUTES[locale][currentLocaleInfo.path];
};
...
export { TranslatedLocaleLink } from './TranslatedLocaleLink';
export { LocaleLink } from './LocaleLink';
export { useLocale } from './useLocale';
export { useLocaleMessages } from './useLocaleMessages';
